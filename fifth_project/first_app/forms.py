...
class contactForm(forms.Form):
    name = forms.CharField(label='Tor name Likh: ')
    email = forms.EmailField(label='Tor email Likh: ',widget=forms.Textarea())
    age = forms.IntegerField(label= 'Boyosh likh: ')
    file = forms.FileField(label= 'Upload Kor: ')
    '''
    name = forms.CharField(label="Full Name : ", help_text="Total length must be within 70 characters",
    required=False, error_messages={'required': 'Please enter your name.'},
    widget = forms.Textarea(attrs = {'id' : 'text_area', 'class' : 'class1 class 2',
